...
- [ ] **I – Clean Code**: Single-responsibility files/functions, naming conventions, no circular imports
- [ ] **II – Test-First / TDD**: Failing tests written and reviewed before implementation begins
- [ ] **III – Tech Stack**: Approved stack only (Next.js · TypeScript · TailwindCSS · Supabase). RLS enabled on all user tables. Service-role key server-side only
- [ ] **IV – Platform UI**: Responsive / Material Design / HIG patterns applied. No hard-coded URLs (all from SCREENFLOW.md). WCAG 2.1 AA met
- [ ] **V – Security-First (OWASP)**: Zod input validation, parameterised queries, HTTP security headers, auth middleware on all routes

**Violations (if any)**:

| Principle | Violation | Justification | Alternative Rejected |
|-----------|-----------|---------------|---------------------|
| [e.g., III] | [e.g., New library] | [Why needed] | [Why existing won't work] |
