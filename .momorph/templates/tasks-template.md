...
- [ ] T002 Initialize Next.js project with TypeScript, TailwindCSS, and Supabase dependencies
- [ ] T003 [P] Configure linting (ESLint), formatting (Prettier), and TypeScript strict mode
- [ ] T004 Load assets from media files as per plan.md instructions (IMPORTANT)
- [ ] T005 [P] Configure HTTP security headers in `next.config.ts` (CSP, HSTS, X-Frame-Options)
- [ ] T006 [P] Create typed `env.ts` module for environment variable access
---
...
- [ ] T007 Setup Supabase schema migrations; enable RLS on all user-data tables
- [ ] T008 [P] Implement auth middleware (`withAuth` wrapper) for route handlers
- [ ] T009 [P] Setup API routing and middleware structure
- [ ] T010 Create Supabase data-access layer (no direct client calls from UI/routes)
- [ ] T011 Configure error handling and structured logging (no PII in logs)
- [ ] T012 Write Zod validation schemas for all shared input types
...
### Tests (US1) ← Write BEFORE implementation (TDD)

- [ ] T020 [P] [US1] Write unit tests for service methods | tests/unit/[feature]/[feature].service.spec.ts
- [ ] T021 [P] [US1] Write integration tests for API endpoints | tests/integration/[feature].spec.ts
- [ ] T022 [P] [US1] Write E2E test scenarios (Playwright) | tests/e2e/[feature].spec.ts

### Implementation (US1) ← After tests are written and approved

- [ ] T023 [P] [US1] Create DTO classes | src/modules/[feature]/dto/
- [ ] T024 [US1] Implement service methods | src/modules/[feature]/[feature].service.ts
- [ ] T025 [US1] Create controller endpoints | src/modules/[feature]/[feature].controller.ts
- [ ] T026 [US1] Add Zod input validation | src/modules/[feature]/validation/
